...
class Args:
    ...
    """if toggled, the backbone network will be trained from scratch"""
    pretrained_backbone_path: str = None
    """the path to the pretrained backbone model"""
    backbone_type: str = None
    """the type of the backbone network, choose from ['sineActor', 'sinelessActor']"""
...
if __name__ == "__main__":
    ...
    if args.agent_type == "classicalNNSineActor":
        ...
        raise ValueError("agent_type must be one of ['classicalNNSineActor', 'classicalNNSinelessActor', 'entangledActor', 'separableActor']")
    
    if args.agent_type in ["classicalNNSineActor", "classicalNNSinelessActor"]:
        assert args.from_scratch is True, "from_scratch must be True for classicalNNSineActor and classicalNNSinelessActor"
        assert args.pretrained_backbone_path is None, "pretrained_backbone_path must be None for classicalNNSineActor and classicalNNSinelessActor"
        assert args.clamp_actor_weights is False, "clamp_actor_weights must be False for classicalNNSineActor and classicalNNSinelessActor"
    if args.agent_type in ["entangled", "separable"]:
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            # fix the reward logging
            # due to the change in https://gymnasium.farama.org/api/wrappers/misc_wrappers/#gymnasium.wrappers.RecordEpisodeStatistics
            # of the latest version of gymnasium
            if "episode" in infos:
                for i in range(args.num_envs):
                    if infos['episode']['_r'][i]:
                        print(f"global_step={global_step}, episodic_return={infos['episode']['r'][i]}")
                        writer.add_scalar("0-Episodic-Stats/episodic_return", infos["episode"]["r"][i], global_step)
                        writer.add_scalar("0-Episodic-Stats/episodic_length", infos["episode"]["l"][i], global_step)
            """

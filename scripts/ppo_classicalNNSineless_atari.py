...
from model import ClassicalPPOAgentWithPlaceholderSineless as Agent
...
class Args:
    ...
    """the path to save the backbone network. If None, defaults to '"trained-models/classical_backbone_(sineless)_{env_id}_latent_{backbone_out_dim}_seed_{seed}.pt"'"""
...
if __name__ == "__main__":
    ...
    if args.backbone_save_path is None:
        ...
        args.backbone_save_path =  f"trained-models/classical_backbone_(sineless)_{args.env_id}_latent_dim_{args.backbone_out_dim}_seed_{args.seed}.pt"
    ...
    run_name = f"{args.wandb_project_name}__{args.exp_name}__{args.env_id}__LatentDim__{args.backbone_out_dim}__{args.seed}__{int(time.time())}"
    print(args)
    if args.track:
        import wandb
    ...
    backbone = Backbone(args.backbone_out_dim).to(device)
    agent = Agent(envs, n_layers=args.n_layers, backbone_out_dim=args.backbone_out_dim, backbone=backbone).to(device)
    print(agent)

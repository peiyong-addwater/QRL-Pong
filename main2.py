...
n_layers_list = [2, 5, 10] # minimum 2 layers for the quantum circuit
...
trained_models_dir = os.path.join("trained-models", "Pong1PModels2")
...
for n_layers in n_layers_list:
    for seed in seeds_list:
        if (n_layers, seed) not in ghz_finished:
            command_list.append(
                f"uv run Pong1PQuantum2.py --num_envs 32 --num_steps 128 --agent_type 'ghz' --seed {seed} --n_layers {n_layers}"
            )
        if (n_layers, seed) not in graph_state_finished:
            command_list.append(
                f"uv run Pong1PQuantum2.py --num_envs 32 --num_steps 128 --agent_type 'graph_state' --seed {seed} --n_layers {n_layers}"
            )
#        if (n_layers, seed) not in separable_finished:
#            command_list.append(
#                f"uv run Pong1PQuantum2.py --num_envs 32 --num_steps 128 --agent_type 'separable' --seed {seed} --n_layers {n_layers}"
#            )
        if (n_layers, seed) not in w_state_finished:
            command_list.append(
                f"uv run Pong1PQuantum2.py --num_envs 32 --num_steps 128 --agent_type 'w_state' --seed {seed} --n_layers {n_layers}"
            )

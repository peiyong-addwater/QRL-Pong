...
def V_i_l(x_i, theta_0, theta_1, theta_2, bias_0, bias_1, bias_2, qubit):
    """
    Single-qubit PQC for data-encoding and parameterized gates.
    """
    qml.U3(theta_0*x_i+bias_0, theta_1*x_i+bias_1, theta_2*x_i+bias_2, wires=qubit)
...
def make_entangled_circ_ghz(n_layers: int)->Callable:
    ...
    def circuit(x, params):
        ...
        for l in range(n_layers - 1):
            for i in range(8):
                V_i_l(x[...,i], params[l][i][0], params[l][i][1], params[l][i][2], params[l][i][3], params[l][i][4], params[l][i][5], i)

...
class PongClassicalAgent64PBackbone(nn.Module):
    """
...
class PongClassicalAgent128PBackbone(nn.Module):
    """
    A classical agent with a classical critic.
    """
    def __init__(self, env):
        ...
        self.observation_dim = env.single_observation_space.shape[0]
        self.backbone = nn.Sequential(
            nn.Linear(self.observation_dim, 8),
            nn.ReLU(),
            nn.Linear(8, 8)  # Output dimension for the critic
        )
        self.actor = PongClassicalPolicy(
            input_dim=8,
            output_dim=self.single_action_dim
...
class PongClassicalAgent256PBackbone(nn.Module):
    """
    A classical agent with a classical critic.
    """
    def __init__(self, env):
        ...
        self.observation_dim = env.single_observation_space.shape[0]
        self.backbone = nn.Sequential(
            nn.Linear(self.observation_dim, 16),
            nn.ReLU(),
            nn.Linear(16, 8)  # Output dimension for the critic
        )
        self.actor = PongClassicalPolicy(
            input_dim=8,
            output_dim=self.single_action_dim
